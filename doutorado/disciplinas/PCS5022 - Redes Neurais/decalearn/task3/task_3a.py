...
APP_PATH = os.getcwd()
print(APP_PATH)
current_path = os.path.join(current_path, os.path.join("data", "dataset.npz"))
...
def default_threshold_classification(y_probs):
 ...
 #multiclass classification using probabilities:
...
def main():
    ...
    if VALIDATE_MODEL:    
        best_ensemble = tuner.get_best_models(5)
        ensemble_classifier(X_val=x_test, y_val=y_test,unseen_data = unseen_data,tuner=tuner)
